...
uint8_t Endstops::cap_state = 0; // 状态 0:普通, 1:锁屏
void Endstops::cap_touch_method()
{
  static millis_t cap_ms = 0, interval_time = 0;
  static uint32_t cap_count = 0;
  static uint8_t cap_num = 0; // 0:等待, 1:触发, 2:结果处理
  const millis_t now_ms = millis();
  if(READ(CAP_TOUCH_PIN)){
    if(!cap_num){
      cap_num = 1;
      cap_ms = now_ms;
    }
  ...
  if(cap_num == 2){
    cap_num = 0;
    uint32_t use_time = now_ms-cap_ms;
    if(2000<use_time && use_time<9999)
    {
      if(cap_count==0){}
      else{}
    }
    else if(5<use_time && use_time<1000)
    {
      // serial_port_index = SERIAL_BOTH;
      // SERIAL_ECHOLNPAIR("s:", now_ms-interval_time);
      interval_time = now_ms;
      cap_count++;
    }
  }

  if((cap_count>0) && ((now_ms-interval_time)>350)){
    switch(cap_count){
      //case 0:break;
      case 1:
        if(cap_state == 1){
          ...
          cap_state = 10;
          infoMenu.menu[++infoMenu.cur] = menuPopuCap;
        }
        break;
      case 2:
        // if(cap_state == 0)
        {
          // cap_state = 10;
          // infoMenu.menu[++infoMenu.cur] = menuPopuCap2;
          uint8_t old_silent = infoSettings.silent;
          infoSettings.silent = !old_silent;
          if(infoSettings.silent)
            ExtUI::onSilentOn();
          else if(infoSettings.silent==0)
            ExtUI::onSilentOff();
        }
        break;
      case 3:break;
