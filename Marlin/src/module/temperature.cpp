...
// public:
  ...
  uint16_t Temperature::heater_mintemp[HOTENDS] = ARRAY_BY_HOTENDS(HEATER_0_MINTEMP, HEATER_1_MINTEMP, HEATER_2_MINTEMP, HEATER_3_MINTEMP, HEATER_4_MINTEMP, HEATER_5_MINTEMP, HEATER_6_MINTEMP, HEATER_7_MINTEMP);
...
#if HAS_CURRENT_SAMPLE
 #if HAS_MOTORS_OCP
  temp_info_t motors_current_info;
 #endif
 #if HAS_HOTEND_OCP
  temp_info_t hotend_current_info;
 #endif
 #if HAS_BED_OCP
  temp_info_t bed_current_info;
 #endif
#endif

#if HAS_FAN
  // fan_frequency = 168000000/56000/30 = 100Hz
  #define FAN_PERIOD      56000
  #define FAN_PRESCALER   30
  ...
  void Temperature::set_fan_speed(uint8_t target, uint16_t speed) {
    ...
    // if((get_MachineFlag()>>MF_FAN)&0x1)
    // {
      if(0<speed&&speed<80) speed = 80; // 255*0.3 ≈ 80 风扇启动速度最低为30%
    // }
    // else{
    //   if(0<speed&&speed<255) speed = 255;
    // }
    uint16_t fan_frequency = (((float)(speed)*(DUTY_RATIO)));
    if(speed > 0){
...
void Temperature::manage_heater() {
  ...
  millis_t ms = millis();
  ...
  #if HAS_CURRENT_SAMPLE
    // if (machine_current.get_machine_current(MOTORS_OCP)>)
  #endif
    ...
    HOTEND_LOOP() {
      ...
      // 根据温度修改led灯颜色
      #if ENABLED(WS2812_LED) && 1
        if(ABS(old_hotend_celsius[e]-temp_hotend[e].celsius)>1.0f){
    ...
    #if ENABLED(WS2812_LED) && 1
      if(need_change && (AUTO_MODE==leds4.get_led_mode())){
        need_change = 0;
        for(uint8_t ci=0; ci<HOTEND_TEMPS; ci++){
          temp_color[ci] = leds4.get_temperature_color(temp_hotend[ci].celsius);
        }
        leds4.set_color(temp_color);
      }
    #endif
...
void Temperature::updateTemperaturesFromRawValues() {
  ...
  TERN_(HAS_MOTORS_OCP, motors_current_info.celsius = (float)machine_current.analog_to_current(MOTORS_OCP, motors_current_info.raw));
  TERN_(HAS_HOTEND_OCP, hotend_current_info.celsius = (float)machine_current.analog_to_current(HOTEND_OCP, hotend_current_info.raw));
  TERN_(   HAS_BED_OCP,    bed_current_info.celsius = (float)machine_current.analog_to_current(   BED_OCP,    bed_current_info.raw));
...
void Temperature::init() {
  // if need use the second set of temperatures
  if(!((get_MachineFlag()>>MF_MAXTEMP)&0x1)){
    #if (HOTENDS>0)
      heater_maxtemp[0] = HEATER_0_MAXTEMP2;
      #if (HOTENDS>1)
        heater_maxtemp[1] = HEATER_1_MAXTEMP2;
        #if (HOTENDS>2)
          heater_maxtemp[2] = HEATER_2_MAXTEMP2;
          #if (HOTENDS>3)
            heater_maxtemp[3] = HEATER_3_MAXTEMP2;
          #endif // (HOTENDS>3)
        #endif // (HOTENDS>2)
      #endif // (HOTENDS>1)
    #endif // (HOTENDS>0)
  ...
  delay(250);
    ...
    #define _TEMP_MIN_E(NR) do{ \
      const int16_t tmin = _MAX(HEATER_ ##NR## _MINTEMP, TERN(HEATitunesER_##NR##_USER_THERMISTOR, 0, (int16_t)pgm_read_word(&HEATER_ ##NR## _TEMPTABLE[HEATER_ ##NR## _SENSOR_MINTEMP_IND].celsius))); \
    ...
    #define _TEMP_MAX_E(NR) do{ \
      int16_t tmax; \
      if((get_MachineFlag()>>MF_MAXTEMP)&0x1){ \
        tmax = _MIN(HEATER_ ##NR## _MAXTEMP, TERN(HEATER_##NR##_USER_THERMISTOR, 2000, (int16_t)pgm_read_word(&HEATER_ ##NR## _TEMPTABLE[HEATER_ ##NR## _SENSOR_MAXTEMP_IND].celsius) - 1)); \
      } \
      else{ \
        tmax = _MIN(HEATER_ ##NR## _MAXTEMP2, TERN(HEATER_##NR##_USER_THERMISTOR, 2000, (int16_t)pgm_read_word(&HEATER_ ##NR## _TEMPTABLE[HEATER_ ##NR## _SENSOR_MAXTEMP_IND].celsius) - 1)); \
      } \
...
void Temperature::update_raw_temperatures() {
  ...
  TERN_(HAS_MOTORS_OCP, motors_current_info.update());
  TERN_(HAS_HOTEND_OCP, hotend_current_info.update());
  TERN_(HAS_BED_OCP,       bed_current_info.update());
...
void Temperature::readings_ready() {
  ...
  TERN_(HAS_MOTORS_OCP, motors_current_info.reset());
  TERN_(HAS_HOTEND_OCP, hotend_current_info.reset());
  TERN_(HAS_BED_OCP,       bed_current_info.reset());
...
void Temperature::tick() {
  ...
  switch (adc_sensor_state) {
    ...
    #if HAS_MOTORS_OCP
      case PrepareCurrentMotors: HAL_START_ADC(MOTORS_OCP_PIN); break;
      case MeasureCurrentMotors: ACCUMULATE_ADC(motors_current_info); break;
    #endif
    #if HAS_HOTEND_OCP
      case PrepareCurrentHotend: HAL_START_ADC(HOTEND_OCP_PIN); break;
      case MeasureCurrentHotend: ACCUMULATE_ADC(hotend_current_info); break;
    #endif
    #if HAS_BED_OCP
      case PrepareCurrentBed: HAL_START_ADC(BED_OCP_PIN); break;
      case MeasureCurrentBed: ACCUMULATE_ADC(bed_current_info); break;
    #endif
