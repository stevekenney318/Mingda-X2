...
#include "../core/debug_out.h"
  ...
  bool MarlinSettings::save() {
    ...
    //
    #if ENABLED(WS2812_LED) && 1
      uint32_t the_led_mode = leds4.get_led_mode();
      EEPROM_WRITE(the_led_mode);
    #endif
    ...
    #if HAS_SHAPING
      #if ENABLED(INPUT_SHAPING_X)
        const float shaping_frequency_x = stepper.get_shaping_frequency(X_AXIS);
        const float shaping_damping_ratio_x = stepper.get_shaping_damping_ratio(X_AXIS);
        EEPROM_WRITE(shaping_frequency_x);
        EEPROM_WRITE(shaping_damping_ratio_x);
      #endif
      #if ENABLED(INPUT_SHAPING_Y)
        const float shaping_frequency_y = stepper.get_shaping_frequency(Y_AXIS);
        const float shaping_damping_ratio_y = stepper.get_shaping_damping_ratio(Y_AXIS);
        EEPROM_WRITE(shaping_frequency_y);
        EEPROM_WRITE(shaping_damping_ratio_y);
      #endif
    #endif
    ...
    //
    #if ENABLED(USE_X_LEVELING)
      const float x_leveling_value = xleveling.get_XLevelingValue();
      EEPROM_WRITE(x_leveling_value);
    #endif
  ...
  bool MarlinSettings::_load() {
    ...
    if (strncmp(version, stored_ver, 3) != 0) {
      ...
      //
      #if ENABLED(WS2812_LED) && 1
        uint32_t the_led_mode = 0;
        EEPROM_READ(the_led_mode);
        if(the_led_mode == AUTO_MODE){
          // leds4.set_color(the_led_mode);
          leds4.set_led_mode(the_led_mode);
        }
        else if(the_led_mode<0x1000000){
          leds4.set_led_mode(the_led_mode);
          leds4.set_color(the_led_mode);
        }
      #endif
      ...
      //
      #if ENABLED(USE_X_LEVELING)
        float x_leveling_value=0.0f;
        EEPROM_READ(x_leveling_value);
        xleveling.set_XLevelingValue(x_leveling_value);
      #endif
...
void MarlinSettings::reset() {
  ...
  //
  #if ENABLED(USE_X_LEVELING)
    xleveling.set_XLevelingValue(0.0f);
  #endif
  ...
  void MarlinSettings::report(const bool forReplay) {
    ...
    // Machine Flag
    CONFIG_ECHO_HEADING("Machine Flags:");
    gcode.report_M1200();
    ...
    //
      ...
      uint32_t the_led_mode = leds4.get_led_mode();
      if(the_led_mode == AUTO_MODE)
        SERIAL_ECHOLNPAIR("The led mode: ", "auto");
      else if(the_led_mode == 0)
        SERIAL_ECHOLNPAIR("The led mode: ", "off");
      else if(the_led_mode>0 && the_led_mode<=0xffffff)
        SERIAL_ECHOLNPAIR("The led color: ", the_led_mode);
    #endif

    //
    // Input Shaping
    //
    TERN_(HAS_SHAPING, gcode.report_M593(forReplay));
