...
static float bed_size[3] = {X_BED_SIZE, Y_BED_SIZE, Z_BED_SIZE};
namespace ExtUI {
  static struct {
  ...
  void setAxisRelative_mm(const float len, const axis_t axis, const feedRate_t feedrate/*=0*/) {
    float min=0, max, position;
    
    max = bed_size[axis];
    position = current_position[axis]+len;
    // constrain(position, min, max);
    current_position[axis] = constrain(position, min, max);
    line_to_current_position(feedrate ?: manual_feedrate_mm_s[axis]);
  }
