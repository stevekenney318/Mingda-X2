...
void LCD_Setup() {
  ...
  LCD_Init();
    ...
    W25Qxx_ReadBuffer(&buf, W25Q64_BOOT_MODE_ADDR, 1);
    // SERIAL_CHAR(buf);
    if(0xff == buf){
      buf = STANDBY_MODE_NUM;
      W25Qxx_WritePage(&buf, W25Q64_BOOT_MODE_ADDR, 1);
      LCD_Sleep_Mode();
      Sys_Enter_Standby();
    }
    // else if(STANDBY_MODE_NUM == buf){
    //   W25Qxx_EraseSector(W25Q64_BOOT_MODE_ADDR);
    // }
  #endif
  boot.scanUpdates();
  // while(1);
  if(boot.scanResetInfoFile()){
    infoSettingsReset();
    storePara();
  }
  if(res == false || boot.scanResetFile()) // Read settings parameter
  {
    TSC_Calibration();
    storePara();
  }
