...
#define TFT_RESETI_FILE "reset_i.txt"
#define BOOT_FILE "boot.bin"

// change name
#define ROOT_DIR_RNAME "CUR35"
#define TFT_RESET_FILE_RNAME "reset.cur"
#define TFT_RESETI_FILE_RNAME "reset_i.cur"
#define BOOT_FILE_RNAME "boot.cur"
...
class TSCBoot {
  public:
    ...
    uint8_t scanHasFile(const char *filename);
    uint8_t scanResetFile(void);
    uint8_t scanResetInfoFile(void);
    uint8_t scanAdminFile(void);
    void scanChangeName(void);
    bool CalibrationBootloader(void);
    uint8_t updateBootloader(uint8_t num);
  private:
