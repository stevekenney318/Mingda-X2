  ...
  if (millis() - nowTime_ms > 500) 
  {
    nowTime_ms = millis();
      ...
      if(READ(CAP_TOUCH_PIN)){
        GUI_DispStringInPrect(&TestEndstopRect[CAP_TOUCH_E], (const uint8_t*)"CTP   T");
        #if PIN_EXISTS(LCD_BACKLIGHT)
          WRITE(LCD_BACKLIGHT_PIN, LOW);
        #endif
      }
      else{
        GUI_DispStringInPrect(&TestEndstopRect[CAP_TOUCH_E], (const uint8_t*)"CTP   O");
        #if PIN_EXISTS(LCD_BACKLIGHT)
          WRITE(LCD_BACKLIGHT_PIN, HIGH);
        #endif
      }
