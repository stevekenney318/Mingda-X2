...
void menuCallBackMove()
{
  ...
  static uint32_t Move_nextTime_z = 0;
  ...
  if(queue.length == 0){    // G命令队列为空
    ...
    // 判断哪个轴移动了
    if(x_add_mm != 0){
      x_add_mm_t = (int)(getAxisPosition_mm(ExtUI::X) + x_add_mm) * 10;
      // 防止超越机器越界
      ...
      queue.enqueue_now_P(G0_STR);    // G命令入队
    }
    else if(y_add_mm != 0){
      y_add_mm_t = (int)(getAxisPosition_mm(ExtUI::Y) + y_add_mm) * 10;
      ...
      queue.enqueue_now_P(G0_STR);
    }
    else if(z_add_mm != 0){
      z_add_mm_t = (int)(getAxisPosition_mm(ExtUI::Z) + z_add_mm) * 10;
  ...
  if((queue.length == 0)){    // G命令队列为空
    isBusy = false;
    if(Move_nextTime_z<now_time)
    {
      Move_nextTime_z = now_time + 200;
      
      if(x_add_mm != 0){
        ExtUI::setAxisRelative_mm(x_add_mm, ExtUI::X);
        x_add_mm = 0;
      }
      else if(y_add_mm != 0){
        ExtUI::setAxisRelative_mm(y_add_mm, ExtUI::Y);
        y_add_mm = 0;
      }
      else if(z_add_mm != 0){
        ExtUI::setAxisRelative_mm(z_add_mm, ExtUI::Z);
        z_add_mm = 0;
      }
    }
  }else isBusy = true;
  ...
  if (now_time - nowTime_ms > 1000) { // Refresh per 1 sec
    nowTime_ms = now_time;
    // Refresh position
