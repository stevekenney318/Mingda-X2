...
#ifdef QUICK_PRINT
const float item_babystep_unit[ITEM_BABYSTEP_UNIT_NUM] = {0.05f, 0.1f, 1};
#else
...
float getBabyStepZAxisTotalMM(){
 #ifdef QUICK_PRINT
  return babystep.axis_total[BS_TOTAL_IND(WHO_AXIS)] / planner.settings.axis_steps_per_mm[WHO_AXIS];
 #else
