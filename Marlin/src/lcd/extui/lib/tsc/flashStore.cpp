...
#ifndef DATE_RESTRICTION
#define DATA_RESTRICTION(min, num, max) MAX(min, MIN(max, num))
#endif

extern uint32_t TSC_Para[7];  // 存放触摸校准数据
extern SETTINGS infoSettings; // 存放部分设置信息
...
bool readStoredPara(void)
{
  ...
  else
  {
    ...
    infoSettings.machine_flags  = byteToWord(data + (index += 4), 4);
  }
  
  return paraExist;
}
// save the infoSettings
void storePara(void)
{
  uint8_t  data[PARA_SIZE]; 
  uint32_t index = 0;
  ...
  wordToByte(infoSettings.machine_flags,  data + (index += 4));
...
// get the machine flag
uint32_t get_MachineFlag(void){
  return infoSettings.machine_flags;
}
// set the machine flag
void set_MachineFlag(uint32_t flag){
  infoSettings.machine_flags = flag;
}
...
void set_infoSettings(char a, uint32_t num){
  switch(a){
    case 'U':{
      infoSettings.rotate_ui = (uint8_t)DATA_RESTRICTION(0, num, 4);
    }break;
  }
}
