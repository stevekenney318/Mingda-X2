...
/*
 * The port that the command was received on  // 接收命令的端口
 */
#if HAS_MULTI_SERIAL
      ...
      if (ISEOL(serial_char)) {
        ...
        if (npos) {
          ...
          if (gcode_N != last_N[i] + 1 && !M110)
            return gcode_line_error(PSTR(STR_ERR_LINE_NO), i+1);
          ...
          if (apos) {
            ...
            if (strtol(apos + 1, nullptr, 10) != checksum)
              return gcode_line_error(PSTR(STR_ERR_CHECKSUM_MISMATCH), i+1);
          }
          else
            return gcode_line_error(PSTR(STR_ERR_NO_CHECKSUM), i+1);
          ...
          if (gpos) {
            switch (strtol(gpos + 1, nullptr, 10)) {
              case 0: case 1:
                ...
                case 5:
              #endif
                PORT_REDIRECT(i+1);                      // Reply to the serial port that sent the command, the 'i+1' is the serial port
        ...
        // Add the command to the queue // the 'i+1' is serial port
