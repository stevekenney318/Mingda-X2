...
#if ENABLED(Z_MULTI_ENDSTOPS) && ENABLED(USE_X_LEVELING)
...
void GcodeSuite::G28() {
  ...
  #if ENABLED(Z_MULTI_ENDSTOPS) && ENABLED(USE_X_LEVELING)
    float x_leveling_value = xleveling.get_XLevelingValue();
    if((x_leveling_value>0.001f) && (!stop_home)){
      xleveling.move_XLeveling_Z1(x_leveling_value);

      set_axis_is_at_home(Z_AXIS);
      sync_plan_position();

      destination[Z_AXIS] = current_position[Z_AXIS];
    } else if((x_leveling_value<-0.001f) && (!stop_home)){
      xleveling.move_XLeveling_Z2(-x_leveling_value);
