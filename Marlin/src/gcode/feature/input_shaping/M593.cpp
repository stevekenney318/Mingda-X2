...
void GcodeSuite::report_M593(const bool forReplay/*=true*/) {
  report_heading_etc(forReplay, PSTR("Input Shaping"));
  #if ENABLED(INPUT_SHAPING_X)
    SERIAL_ECHOLNPAIR_P("  M593 X"
...
void GcodeSuite::M593() {
  if (!parser.seen_any()) return report_M593();
  ...
  if (parser.seen('F')) {
    ...
    if (freq == 0.0f || freq > min_freq) {
      ...
      SERIAL_ECHOLNPAIR_P("?Frequency (F) must be greater than ", min_freq, " or 0 to disable");
    }
  }
}
