...
#define SERIAL_BOTH 0x7F
  ...
  #define _PORT_RESTORE(n)      RESTORE(n)
    ...
    #define SERIAL_OUT(WHAT, V...) do{ \
       if ( serial_port_index == 0 || serial_port_index == 1 || serial_port_index == SERIAL_BOTH) (void)MYSERIAL0.WHAT(V); \
       if ( serial_port_index == 2 || serial_port_index == SERIAL_BOTH) (void)MYSERIAL1.WHAT(V); \
       if ( serial_port_index == 3 || serial_port_index == SERIAL_BOTH) (void)MYSERIAL2.WHAT(V); \
    }while(0)
   #else
    #define SERIAL_OUT(WHAT, V...) do{ \
